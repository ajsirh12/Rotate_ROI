...
def rotate_test(x, y, w, h, nw, nh):
    ...
    if 0 <= ANGLE <= 90:
        ...
        print("2.", int(xx), int(-yy) + int(asd))

    elif 90 < ANGLE <=180:
        asd = np.sin(np.pi * ((ANGLE - 90)/180))
        asd = asd * w

        print("2.", int(xx) + int(asd), int(-yy) + nh)

    elif 180 < ANGLE <=270:
        asd = np.cos(np.pi * ((ANGLE - 180)/180))
        asd = asd * h

        print("2.", int(xx) + nw, int(-yy) + int(asd))

    elif 270 < ANGLE <=360:
        asd = np.cos(np.pi * ((ANGLE - 270)/180))
        asd = asd * h

        print("2.", int(xx) + int(asd), int(-yy))
        ...
        if file.endswith('.xml'):
            ...
            with open(yolo_format, "w") as f:
                ...
                for item in objects:
                    ...
                    # print('1.', cy, cx)
                    cx, cy = rotate_matrix(cx, cy, width, height)
                    # print('4.', cy, cx)
